...
def Gain_radio(data, a, target):
    ...
    IV = 0.0
    for name in data[a].unique():
        p = attributes[name] / length
        if p != 0:
            IV -= p * np.log2(p)
    if IV == 0:
        gain_radio = 0
    else:
        gain_radio = Gain(data, a, target) / IV
    return gain_radio
...
def chooseBestFeature(data):
    ...
    for feature in data.columns[:-1]:
        print('属性', feature)
        gain_radio = Gain_radio(data, feature, "好瓜")

        print("信息增益率为", gain_radio)
        print()
        if gain_radio > gain_radio_max:
            gain_radio_max = gain_radio
            pos_best = pos
            feature_best = feature

    print("最大信息增益率为：", gain_radio_max, "属性名称：", feature_best)
