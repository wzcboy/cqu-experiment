...
# 将标签转化为向量
Y = []
code = [[0 for i in range(10)] for j in range(10)]
for i in range(10):
    code[i][i] = 1

label2Vec = {'0': code[0], '1': code[1], '2': code[2], '3': code[3], '4': code[4], '5': code[5],
             '6': code[6], '7': code[7], '8': code[8], '9': code[9]}
...
def BP_accumulate(input_d, input_q, input_l):
    ...
    while True:
        ...
        if np.abs(E - lastE) < 0.000001 or count > 1000:
            break
        lastE = E
    ##Loss可视化
    plt.figure()
...
# w, theta, v, gamma, count = BP_standard(n, n + 1, 10)
# print("训练结束!")
# print("迭代次数为：", count, "\n")
w, theta, v, gamma, count = BP_accumulate(n, n + 1, 10)
